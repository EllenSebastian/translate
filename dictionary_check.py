...
import translation_dictionary, codecs

def _get_list_of_words(sentence, delims, remove):
...
keys = [key.lower() for key in translation_dictionary.vocab.keys()]
with codecs.open('corpus/corpus.french.all', 'r', 'utf-8') as f:
  for line in f:
    for word in _get_list_of_words(line, delims=" '", remove=u'\n.,?»«'):
